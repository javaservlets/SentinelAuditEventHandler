...
/**
 * The handler publishes audit events formatted using {@link SyslogFormatter} to a sentinel daemon using
 * the configured {@link SyslogPublisher}. The publisher is flushed after each write.
 */
public class SentinelAuditEventHandler extends AuditEventHandlerBase {

    private static final Logger logger = LoggerFactory.getLogger(SentinelAuditEventHandler.class);
     ...
     */
    @Inject
    public SentinelAuditEventHandler(
            final SentinelAuditEventHandlerConfiguration configuration,
    ...
    public Promise<ResourceResponse, ResourceException> publishEvent(Context context, String topic, JsonValue event) {
        ...
        try { //rj? could set a breakpt right below
